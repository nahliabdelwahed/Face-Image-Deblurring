...
class Generator:
  def __init__(self, name, is_training, skip, ngf=64, norm='instance', image_size=128):
    self.name = name
    self.reuse = False
    self.ngf = ngf
    self.norm = norm
  ...
  def __call__(self, input):
    ...
    with tf.variable_scope(self.name):
      # conv layers
      c7s1_32 = ops.c7s1_k(input, input, self.ngf, is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, skip=self.skip, name='c7s1_32')                             # (?, w, h, 32)
      ...
      if self.image_size <= 128:
        ...
        res_output = ops.n_res_blocks(d128, reuse=self.reuse, n=9)      # (?, w/4, h/4, ?)
      ...
      output = ops.c7s1_k(u32, input, 3, norm=None,
